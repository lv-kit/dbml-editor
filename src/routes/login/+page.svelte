<script lang="ts">
	...
	const microsoftProvider = $derived(data.providers.find((p) => p.id === 'microsoft-entra-id'));
	const emailProvider = $derived(data.providers.find((p) => p.id === 'resend'));

	async function handleEmailLogin(event: Event) {
		event.preventDefault();
		if (!email) return;

		// Trigger Auth.js sign in with email
